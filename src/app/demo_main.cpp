...
// Minimal product demo (2026-09-03): open the real Audient ASIO driver at
// 48 kHz / 64 samples, route the physical mic input through the engine graph to
// Output 1/2 with the processed-mic monitor branch, and expose live counters and
// a few keyboard controls. This is NOT production audio logic — it is a "what is
// the current state" harness on top of the Phase 2 engine.
...
const wchar_t kEditorHostClass[] = L"AudientConsoleVst3EditorHost";
...
{
    // The editor host is a native plug-in window; a DPI-unaware process gets
    // bitmap-stretched (blurry/wrong scale) at 125/150/200%. Opt into per-monitor
    // DPI awareness so plug-in views render at physical pixels (project guidelines §15).
    ...
    SetConsoleTitleA("Audient Console - Phase 2 engine demo");

    // Optional non-interactive auto-exit: `audient_console_demo --seconds 10`
    long autoExitSeconds = 0;
    for (int i = 1; i < argc - 1; ++i)
    {
        if ((strcmp(argv[i], "--seconds") == 0 || strcmp(argv[i], "-s") == 0))
        {
    ...
    // Optional VST3 module on the mic chain: `audient_console_demo --vst3 <path>`
    std::string vst3ModulePath;
    for (int i = 1; i < argc - 1; ++i)
    {
        if (strcmp(argv[i], "--vst3") == 0)
        {
            vst3ModulePath = argv[i + 1];
            break;
        }
    }

    // Optional directory used by the interactive plugin chooser.
    std::string vst3Dir = "C:\\Program Files\\Common Files\\VST3";
    for (int i = 1; i < argc - 1; ++i)
    {
        if (strcmp(argv[i], "--vst3dir") == 0)
        {
            vst3Dir = argv[i + 1];
            break;
        }
    ...
    std::printf("Audient Console demo (Phase 2 engine)\n");
    std::printf("  enu điều kiện: Audient ASIO driver + iD14 connected, 48 kHz / 64 samples\n\n");
    ...
    for (const audient::asio::DriverIdentity& candidate : available)
    {
    ...
    std::printf("ASIO subsystems initialized. Opening driver...\n\n");
    ...
    config.inputTrimDb = 0.0f;
    config.micMonitorDb = -6.0f; // processed mic monitor at a moderate level
    config.micMonitorMute = false;
    config.downlinkMute = true; // no Windows system mix yet in this demo
    config.physicalOutputDb = 0.0f;
    config.outputHeadroomDb = 3.0f;
    config.revision = 1;
    graph.publishConfig(config);

    // Optional VST3 mic-chain effect(s). Interaction:
    //   --vst3 <path>   : explicit module/bundle (no menu), added as slot 0
    //   (none, interactive): menu of *.vst3 under --vst3dir (default Common Files\VST3)
    //   During streaming: 'a' adds from the directory, 'r' removes the last
    //   slot, 'b' toggles that slot's bypass, 'B' toggles whole-chain bypass.
    //
    // The mic chain is a Vst3Chain with atomic snapshot swap (realtime-safe):
    // the ASIO callback only reads the published snapshot; add/remove/bypass
    // happen on this UI thread, retired snapshots are reaped after a block
    // grace, and removed processors are destroyed only after the chain settles.
    ...
    const auto rebuildMicChain = [&]() {
        ...
        {
            chainSlots.push_back({slot.processor.get(), slot.bypass});
        }
    ...
    const auto addPluginFromPath = [&](const std::string& path) -> int {
        ...
        slot.processor = slot.host->createEffectProcessor(*effect, 48000.0, static_cast<long>(graph.maxBlockSamples()),
        ...
        std::printf("[vst3] effect \"%s\" added as slot %d (latency %u samples)\n", effect->name.c_str(), index,
                    static_cast<unsigned>(pluginSlots[static_cast<std::size_t>(index)].processor->latencySamples()));
        rebuildMicChain();
        return index;
    };

    if (vst3ModulePath.empty() && autoExitSeconds == 0 && !vst3Candidates.empty())
    {
        // Interactive chooser before streaming starts (std::cin based).
        ...
        {
            vst3ModulePath = vst3Candidates[choice];
            std::printf("[vst3] picked %s\n", vst3ModulePath.c_str());
        }
        else
        {
    ...
    if (!vst3ModulePath.empty() && addPluginFromPath(vst3ModulePath) < 0)
    {
        std::printf("[vst3] initial plug-in not loaded; continuing with an empty chain\n");
    }

    graph.setMicChain(&audient::vst3::Vst3Chain::processMono, &micChain);
    std::printf("[vst3] mic chain wired (%zu slot(s) ready, bypass %s)\n", pluginSlots.size(),
    ...
    std::printf("STREAM STARTED. Mic monitor (post-gain) is routed to Output 1/2.\n\n");
    ...
    {
        std::printf("  (non-interactive: auto-exit after %ld s)\n", autoExitSeconds);
    }
    std::printf("\n");
    ...
    auto uiThread = std::thread([&]() {
        while (running.load(std::memory_order_relaxed))
        {
            ...
            const float micPeak = bridge.micUplinkPeak();
            ...
            const bool chainBypass = chainBypassStatus.load(std::memory_order_relaxed);
            std::printf("\rstate=%-11s callbacks=%-9llu xruns=%-5llu overloads=%-5llu mic=%6.1fdBFS last=%-7.3fms p95=%-7.3fms slots=%zu/bypass=%d  ",
                        state == audient::asio::DeviceState::Streaming ? "Streaming"
                        : state == audient::asio::DeviceState::Ready    ? "Ready"
                        : state == audient::asio::DeviceState::Reconnecting ? "Reconnecting"
                        : state == audient::asio::DeviceState::NoDevice ? "NoDevice"
                                                                         : "Other",
                        ...
                        static_cast<double>(micDb), backend.lastCallbackMs(), backend.p95CallbackMs(),
                        slots, chainBypass ? 1 : 0);
            std::fflush(stdout);
    ...
    bool quit = false;
    const auto autoExitDeadline = autoExitSeconds > 0
        ? std::chrono::steady_clock::now() + std::chrono::seconds(autoExitSeconds)
        : std::chrono::steady_clock::time_point::max();

    // Native editor hosting (VST-010, demo path). Only valid when at least one
    // slot was loaded; everything here runs on the demo's UI thread, never the
    // ASIO callback. The editor targets the last (highest) active slot.
    audient::vst3::Vst3Editor pluginEditor;
    EditorHostState editorHostState{};
    HWND editorHostWindow = nullptr;

    // Active slot whose host routes its controller's performEdit into the
    // slot's processor parameter queue (VST-009). Set on editor open, cleared
    // on editor close/remove. We track the host pointer (owned by the slot's
    // unique_ptr, stable while the editor is open; 'r' closes the editor
    // before destroying the slot).
    ...
    while (!quit)
    {
        ...
        if (autoExitSeconds == 0 && _kbhit())
        {
            const int key = tolower(_getch());
            switch (key)
            {
            case '+':
            case '=':
            {
                audient::engine::EngineConfig next = graph.configSnapshot();
                next.physicalOutputDb = std::min(12.0f, next.physicalOutputDb + 1.0f);
                next.revision = next.revision + 1u;
                graph.publishConfig(next);
                std::printf("\noutput +1 dB -> %.1f dB\n", next.physicalOutputDb);
                break;
            }
            case '-':
            case '_':
            {
                audient::engine::EngineConfig next = graph.configSnapshot();
                next.physicalOutputDb = std::max(-60.0f, next.physicalOutputDb - 1.0f);
                next.revision = next.revision + 1u;
                graph.publishConfig(next);
                std::printf("\noutput -1 dB -> %.1f dB\n", next.physicalOutputDb);
                break;
            ...
            {
                audient::engine::EngineConfig next = graph.configSnapshot();
                next.micMonitorMute = !next.micMonitorMute;
                next.revision = next.revision + 1u;
                graph.publishConfig(next);
                std::printf("\nmonitor %s\n", next.micMonitorMute ? "MUTED" : "UNMUTED");
                break;
            }
            case 'p':
                backend.panic();
            ...
            case 'b':
                ...
                {
                    PluginSlot& slot = pluginSlots.back();
                    slot.bypass = !slot.bypass;
                    std::printf("\nslot %zu bypass %s\n", pluginSlots.size() - 1, slot.bypass ? "ON" : "OFF");
                    rebuildMicChain();
                }
                break;
            case 'B':
            ...
            case 'e':
                ...
                {
                    // Let the user choose which slot's editor to open.
                    ...
                    if (pluginSlots.size() > 1)
                    {
                        ...
                        for (std::size_t i = 0; i < pluginSlots.size(); ++i)
                        {
                            std::printf("  [%3zu] %s%s%s\n", i, pluginSlots[i].name.c_str(),
                                        pluginSlots[i].bypass ? "  (bypassed)" : "", i == pluginSlots.size() - 1 ? "  <- last" : "");
                        }
                        std::printf("> ");
                        std::fflush(stdout);
                        const std::string line = readLineFromConsole();
    ...
    while (std::chrono::steady_clock::now() < mutedDeadline && !bridge.fadeIsMuted())
    {
    ...
    std::printf("\ndemo finished. callbacks=%llu xruns=%llu overloads=%llu\n",
                static_cast<unsigned long long>(backend.callbackCount()),
                static_cast<unsigned long long>(backend.xrunCount()),
                static_cast<unsigned long long>(backend.overloadCount()));
    return 0;
