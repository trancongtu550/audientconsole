...
public:
    ...
    void gracefulTeardown()
    {
        ...
        m_session->adapter->requestFadeOut();
        const auto deadline = std::chrono::steady_clock::now() + std::chrono::milliseconds(3000);
        while (std::chrono::steady_clock::now() < deadline && !m_session->adapter->fadeIsMuted())
        {
            std::this_thread::sleep_for(std::chrono::milliseconds(5));
        }
        std::string error;
        if (!m_session->backend->stop(error))
        {
...
private:
    bool buildAndStart(std::unique_ptr<StreamSession>& out, std::string& error, bool verbose)
    {
        ...
        // Buffer: preserve the requested --buffer, else the first-negotiated
        // buffer, else 64 -> 128 -> driver preferred (never force unsupported).
        long buffer = 0;
        long desired = m_bufferArg > 0 ? m_bufferArg : m_firstBuffer;
        if (desired > 0 && desired >= caps.bufferSizes.minimum && desired <= caps.bufferSizes.maximum)
        {
            std::string cfgError;
            if (session->backend->configure(48000, desired, plan, cfgError))
            {
                buffer = desired;
            }
            else if (verbose)
            {
                std::printf("configure(48000,%ld) failed (%s)\n", desired, cfgError.c_str());
            }
        }
        if (buffer == 0)
        {
            ...
            for (const long candidate : candidates)
            {
                ...
                std::string cfgError;
                if (session->backend->configure(48000, candidate, plan, cfgError))
                {
                    buffer = candidate;
                    break;
                }
        ...
        if (verbose)
        {
            std::printf("CONFIG: requested 48 kHz / %ld samples / mic-in=%ld\n", buffer, plan.micInput);
        }
...
int main(int argc, char** argv)
{
    ...
    bool useGui = false;     // --gui: lightweight Win32 diagnostic panel (control thread only)
    ...
    // Initial chain from repeatable --vst3 in order. When no --vst3 is given on
    // an interactive console, a native file picker asks for ONE plug-in to start
    // with (more can be added live later with the 'a' key).
    if (vst3Paths.empty() && tui.active())
    {
    ...
    feeder.attachSink(feederSink, connectedGeneration);
    if (connectedGeneration != 0)
    {
    ...
    auto publishChannelSends = [&](audient::asio::AsioRoutingAdapter& adapter) {
        ...
        if (dualMode)
        {
            ...
            s0.virtualMicSend = sendFor(true, 0.0f, false, 1);
            ...
            s1.virtualMicSend = sendFor(false, 0.0f, true, 1);
    ...
    if (useGui)
    {
        ...
        dcfg.getAsioText = [&]() -> std::string {
            auto s=asioStream.snapshotStatus();
            char b[128]; std::snprintf(b,sizeof(b),"ASIO %s 48k/%ld", s.stateText.c_str(), s.buffer);
            return b;
        };
        ...
        diagHwnd = DiagGui_Create(GetModuleHandleW(nullptr), dcfg);
    ...
    while (!g_stop.load(std::memory_order_relaxed) && !quit.load(std::memory_order_relaxed))
    {
        ...
        // Callback rate -> actual buffer (per-window dt aware).
    ...
    const bool bypassed = chainBypass.load(std::memory_order_relaxed);
        ...
        if (isTui)
        {
            ...
            // (re-reading hardware, never restoring stale software volume).
            // --no-hardware disables the whole layer (controlled A/B pop tests).
            ...
            if (!noHardware && !hw.isOpen() && hwNow >= hwRetryAt)
            {
                ...
                if (openRes != audient::hardware::Result::Ok)
                {
                    if (!hwOpenFailureLogged)
                    {
                        hwOpenFailureLogged = true;
                        const std::string err = hw.lastError();
                        std::printf("[hardware] open failed: %s%s%s\n",
                ...
                else
                {
                    ...
                    std::printf("[hardware] open OK - iD14 device volume control ready\n");
                }
            }
            ...
            if (dualMode)
            {
                ...
                std::snprintf(dual, sizeof(dual),
                              "Dual Ch1 RAW %s dBFS | POST %s dBFS || Ch2 RAW %s dBFS | POST %s dBFS  (%u in, cfg %zu, vm=ch1 only, focus %s)",
        ...
        else
        {
            ...
            if (dualMode)
            {
                std::printf("[dual] ch1 RAW %.1fdBFS POST %.1fdBFS | ch2 RAW %.1fdBFS POST %.1fdBFS  (plan %u in, cfg %zu, vm=ch1 only)\n",
            ...
            std::printf(
                "[status] asio=%s rate=48000 reqbuf=%ld actbuf=%ld cb=%llu xruns=%llu ovl=%llu last=%.3fms p95=%.3fms "
                ...
                " | device phys=%s rec=%llu lost=%llu att=%llu",
                snap.stateText.c_str(), snap.buffer, actualBuffer,
    ...
    std::printf("final: asio callbacks=%llu xruns=%llu overloads=%llu "
                ...
                "| device losses=%llu recoveries=%llu failed-attempts=%llu\n",
