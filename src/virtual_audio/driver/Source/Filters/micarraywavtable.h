...
#define MICARRAY_32_BITS_PER_SAMPLE_PCM         32      // 32 Bits Per Sample (float container)
...
{
    // 48 KHz 32-bit PCM 2 channels (Q5-A3B endpoint format).
    //
    // The internal app<->driver transport contract stays MONO float32
    // (CaptureRingContract.h); the Windows WaveRT endpoint is the proven-present
    // 2ch 32-bit int PCM format and AudientCaptureSourceFillContiguous converts
    // mono float32 -> signed PCM32 L/R (clamped [-1,+1], L=R) in the DMA fill.
    // See audientcapturesource.cpp. Native IEEE_FLOAT endpoint presentation is a
    // deferred investigation (deferred; not a blocker for A3B).
    {
        {
...
{
    // 48 KHz 32-bit PCM 2 channels (provable-present device range; the ring is
    // mono float32 and the DMA fill converts to signed PCM32 L/R).
    {
        {
