...
//
// g_RegistryPath is used to store the registry settings path for the driver.
//
...
--*/
{
    ...
    //
    // Tear down the capture control plane (control device + interface).
    //
...
--*/
    ...
    //
    // Install the capture control plane (stable device-interface GUID + IOCTLs
    // for the shared capture ring transport). Must run AFTER PcInitializeAdapterDriver
    // so we save the portcls dispatch handlers we forward non-control IRPs to.
    //
    ntStatus = AudientControlPlaneInitialize(DriverObject);
    IF_FAILED_ACTION_JUMP(
        ntStatus,
        DPF(D_ERROR, ("AudientControlPlaneInitialize failed 0x%x", ntStatus)),
        Done);
...
--*/
    ...
    //
    // Register + enable the stable capture-control-plane device-interface GUID
    // on the PHYSICAL device object (SetupAPI discovery for the IOCTL channel).
    // Non-fatal: the control device symlink still exposes the channel.
    //
    {
        NTSTATUS cpStatus = AudientControlPlaneRegisterInterface(pAdapterCommon->GetPhysicalDeviceObject());
        if (!NT_SUCCESS(cpStatus))
        {
            DPF(D_TERSE, ("AudientControlPlaneRegisterInterface deferred/non-fatal 0x%x", cpStatus));
        }
    }
...
#pragma warning(suppress: 28118)
    ...
    case IRP_MN_STOP_DEVICE:
        //
        // Stop publishing the capture ring: the DMA path serves digital silence
        // while the device is stopped/removed. The control device + interface
        // survive a STOP so a later START can CONNECT again.
        //
